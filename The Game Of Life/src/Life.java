import java.awt.BorderLayout;
import java.awt.Color;
import java.awt.Container;
import java.awt.FlowLayout;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import java.awt.event.WindowAdapter;
import java.awt.event.WindowEvent;
import java.io.FileNotFoundException;
import java.io.IOException;

import javax.swing.JButton;
import javax.swing.JFrame;
import javax.swing.JPanel;
import javax.swing.JComboBox;

/*
    this is the Controller component
 */

class Life extends JFrame implements ActionListener
{
	private static final long serialVersionUID = 1L;
	private LifeView view;
	private LifeModel model;
	private JButton runButton, pauseButton, resumeButton, resetButton, nextFrame;
	private String[] colors = { "Blue", "Red", "Black", "Random" };
	private String[] select = { "random", "blinker", "glgun13", "penta", "rotate", "static", "tumbler" };
	@SuppressWarnings("rawtypes")
	private JComboBox colorList, startSelector;

	/** construct a randomized starting grid */
	Life() throws IOException
	{
		this(null);
	}
	
	/** construct a grid using the info in text file */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	Life(String fileName) throws IOException
	{
		super("Conway's Life");

		// build the buttons and drop down menus
		JPanel controlPanel = 
				new JPanel(new FlowLayout(FlowLayout.CENTER));
		runButton = new JButton("Run");
		runButton.addActionListener(this);
		runButton.setEnabled(true);
		controlPanel.add(runButton);
		pauseButton = new JButton("Pause");
		pauseButton.addActionListener(this);
		pauseButton.setEnabled(false);
		controlPanel.add(pauseButton);
		resumeButton = new JButton("Resume");
		resumeButton.addActionListener(this);
		resumeButton.setEnabled(false);
		controlPanel.add(resumeButton);
		nextFrame = new JButton("Next Frame");
		nextFrame.addActionListener(this);
		nextFrame.setEnabled(true);
		controlPanel.add(nextFrame);
		resetButton = new JButton("Reset");
		resetButton.addActionListener(this);
		resetButton.setEnabled(false);
		controlPanel.add(resetButton);
		colorList = new JComboBox(colors);
		colorList.addActionListener(this);
		colorList.setEnabled(true);
		controlPanel.add(colorList);
		startSelector = new JComboBox(select);
		startSelector.addActionListener(this);
		startSelector.setEnabled(true);
		controlPanel.add(startSelector);

		// build the view
		view = new LifeView();
		view.setBackground(Color.white);

		// put buttons, view together
		Container c = getContentPane();
		c.add(controlPanel, BorderLayout.NORTH);
		c.add(view, BorderLayout.CENTER);

		// build the model
		model = new LifeModel(view, fileName);
	}

	public void actionPerformed(ActionEvent e)
	{
		if(e.getSource() == colorList)
		{
			int c = colorList.getSelectedIndex();
			view.colorUpdate(c);
		}
		else if(e.getSource() == startSelector)
		{
			runButton.setEnabled(true);
			pauseButton.setEnabled(false);
			resumeButton.setEnabled(false);
			nextFrame.setEnabled(true);
			resetButton.setEnabled(false);
			int c = startSelector.getSelectedIndex();
			try {
				model.switchMap(c);
			} catch (FileNotFoundException e1) {
				e1.printStackTrace();
			}
		}
		else
		{
			JButton b = (JButton)e.getSource();
			if ( b == runButton )
			{
				model.run();
				runButton.setEnabled(false);
				pauseButton.setEnabled(true);
				resumeButton.setEnabled(false);
				nextFrame.setEnabled(false);
				resetButton.setEnabled(true);
			}
			else if ( b == pauseButton )
			{
				model.pause();
				runButton.setEnabled(false);
				pauseButton.setEnabled(false);
				resumeButton.setEnabled(true);
				nextFrame.setEnabled(true);
				resetButton.setEnabled(true);
			}
			else if ( b == resumeButton )
			{
				model.resume();
				runButton.setEnabled(false);
				pauseButton.setEnabled(true);
				resumeButton.setEnabled(false);
				nextFrame.setEnabled(false);
				resetButton.setEnabled(true);
			}
			else if( b == nextFrame)
			{
				model.updateFrame();
			}
			else if( b == resetButton)
			{
				try {
					model.reset();
				} catch (FileNotFoundException e1) {
					e1.printStackTrace();
				}
				runButton.setEnabled(true);
				pauseButton.setEnabled(false);
				resumeButton.setEnabled(false);
				resetButton.setEnabled(false);
				nextFrame.setEnabled(true);
			}
		}
	}
	
	public static void main(String[] args) throws IOException
	{
		Life conway = new Life(); //parameterize to start w/ a particular file
		
		conway.addWindowListener(new WindowAdapter()
		{
			public void windowClosing(WindowEvent e)
			{
				System.exit(0);
			}
		});
		conway.setSize(630, 690);
		//conway.show(); //deprecated, added call below
		conway.setVisible(true);
	}
}