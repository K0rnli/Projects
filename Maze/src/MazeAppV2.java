...
public class MazeAppV2 extends JFrame implements ActionListener {
	...
	public MazeAppV2() {
		...
		fontPanel.add(new JLabel("Step Count: "), "West");
	...
	private void doCount() {
		...
		if (newValue>0)
			count = newValue;
		fontField.setText(Integer.toString(count));
		mazeDisplay.setFont(new Font("Courier New",Font.BOLD, count));
		pathDisplay.setFont(new Font("Courier New",Font.BOLD, 16));
	...
	private void doStep() {
		...
		}while(!solver.isSolved() && timerInterval == 0);
		updateMaze();
	}
	...
	private void toggleSolverType() {
		String label = solverType.getText();
		if (label.equalsIgnoreCase("stack")) {
			if ( mazeLoaded ) {
				solverType.setText("queue");
				reset();
			}
		} else if (label.equalsIgnoreCase("queue")) {
			if ( mazeLoaded ) {
	...
	private void makeNewSolver() {
		if(solverType.getText().equalsIgnoreCase("queue"))
			solver = new MazeSolverQueue(this.maze);
		else
			solver = new MazeSolverStackV2(this.maze);
	}
	...
	public static void main(String[] args) {
		...
		MazeAppV2 maze = new MazeAppV2();
		maze.setSize(810, 1010);
	}
